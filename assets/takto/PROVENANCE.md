...
| `turntable.webp` | `TAKTO-TURNTABLE.mp4` (1600 × 900, 20 fps, 24 s) | full frame, no crop, 1080 px wide, 15 fps, q 68, loops (2.8 MB) |

Both are renders from the release CAD, not photographs of the bench. A cropped, faster
turntable was tried and rejected: the crop clipped the fingers on the side views. The full
turn at full frame is worth its 2.8 MB.
...
ffmpeg -y -i docs/media/TAKTO-TURNTABLE.mp4 \
  -vf "fps=15,scale=1080:-1:flags=lanczos" \
  -c:v libwebp -lossless 0 -q:v 68 -compression_level 6 -loop 0 -an $OUT/turntable.webp
```
